# Module 2 - ANSI SQL Using MySQL - Answers

## Database Setup

First we need to create all the tables and insert the sample data.
...
```sql
SELECT e.title, COUNT(s.session_id) AS sessions_in_peak
FROM Events e
...
```sql
SELECT s1.title AS session_1, s2.title AS session_2, s1.event_id
FROM Sessions s1
...
```sql
SELECT u.full_name, e.title, COUNT(*) AS times_registered
FROM Registrations r
